  ...
  if(embedded){
    return <div className="min-h-0 flex-1 overflow-hidden px-6 pb-8 pt-2">
      {sources.length?<div className="grid h-full min-h-0 grid-cols-[280px_1fr] overflow-hidden rounded-3xl bg-white shadow-[0_8px_30px_rgba(33,40,70,.05)]">
        ...
        <main data-source-library-content={subjectId} className="min-h-0 overflow-y-auto bg-[#F6F7FA] p-6">
          {subjectId === "math" && <style>{`[data-source-library-content="math"] > div > .mt-3.grid.grid-cols-2.gap-3 > article:nth-child(n+3){display:none}`}</style>}
