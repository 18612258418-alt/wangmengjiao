...
export function StudyTaskModal({ kind, onClose }: { kind: StudyTaskKind; onClose: () => void }) {
  ...
  if (kind === "math-homework") return <div className="fixed inset-0 z-[625] flex h-[100dvh] flex-col bg-[#E9EBF0]">
    <header className="flex h-[64px] shrink-0 items-center border-b border-[#DDE0E7] bg-white px-5 shadow-[0_1px_5px_rgba(24,31,54,.04)]">
      <button onClick={onClose} aria-label="关闭作业" className="grid h-10 w-10 place-items-center rounded-xl hover:bg-[#F3F4F7]"><X size={20}/></button>
      <div className="ml-3"><b className="block text-[14px] text-[#202431]">定积分作业 · 第 6 次</b><span className="text-[9px] text-[#9299A7]">高等数学 · 共 3 题</span></div>
      <div className="ml-auto flex items-center gap-3"><PenToolbar layout="inline" showMarker={false} tool={tool} onToolChange={setTool}/><button onClick={clear} className="rounded-full bg-[#F3F4F7] px-4 py-2 text-[9px] text-[#737B89]">清空</button></div>
    </header>
    <main className="grid min-h-0 flex-1 grid-cols-[64%_36%]">
      <section className="min-h-0 overflow-y-auto border-r border-[#DDE0E7] bg-[#ECEEF4] p-6">
        <div className="relative mx-auto aspect-[210/297] w-full max-w-[760px] overflow-hidden bg-white px-[8%] py-[7%] shadow-[0_10px_35px_rgba(34,40,65,.12)]">
          <div className="pointer-events-none relative z-10 flex items-start justify-between border-b border-[#D9DDE5] pb-5"><div><p className="text-[10px] font-semibold text-[#5967D9]">高等数学 · 第 6 次作业</p><h1 className="mt-2 text-[24px] font-bold">定积分与换元积分</h1></div><span className="text-[9px] text-[#979EAB]">第 1 页 / 共 1 页</span></div>
          <div className="pointer-events-none relative z-10 mt-6 space-y-7">{([1,2,3] as const).map((number)=><div key={number} className="min-h-[170px] border-b border-[#E5E7EC] pb-6"><div className="flex items-start gap-3"><b className={`grid h-7 w-7 shrink-0 place-items-center rounded-full text-[10px] ${selectedQuestion===number?"bg-[#4D5CFF] text-white":"bg-[#F0F1F5] text-[#596170]"}`}>{number}</b><p className="text-[12px] leading-6 text-[#303642]">{MATH_QUESTIONS[number]}</p></div><div className="ml-10 mt-4 h-[88px] bg-[linear-gradient(transparent_31px,#E6E9F0_32px)] bg-[length:100%_32px]"/></div>)}</div>
          <canvas ref={workCanvas} className="absolute inset-0 z-20 h-full w-full touch-none" style={{cursor:tool==="eraser"?"cell":"crosshair"}} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerLeave={up} onPointerCancel={up}/>
        </div>
      </section>
      <section className="flex min-h-0 flex-col bg-white"><div className="min-h-0 flex-1 overflow-y-auto p-6"><div className="flex gap-2">{([1,2,3] as const).map(number=><button key={number} onClick={()=>{setSelectedQuestion(number);setGraded(false);}} className={`rounded-full px-4 py-2 text-[10px] font-semibold ${selectedQuestion===number?"bg-[#4D5CFF] text-white":"bg-[#F1F2F6] text-[#737B89]"}`}>第 {number} 题</button>)}</div><h2 className="mt-6 text-[19px] font-bold">第 {selectedQuestion} 题作答</h2><p className="mt-2 text-[10px] leading-6 text-[#737B89]">直接在左侧作业纸上书写。批改时会检查计算过程、积分区间与最终结果。</p>{inkFeedback&&<div className="mt-5 rounded-2xl bg-[#F1F4FF] p-4 text-[10px] leading-6 text-[#53628B]">{inkFeedback}</div>}{graded&&<div className="mt-5 rounded-2xl bg-[#EEF7F2] p-4"><b className="text-[11px] text-[#34805B]">已完成批改</b><p className="mt-2 text-[10px] leading-6 text-[#4E665A]">换元方向正确。提交前请再确认换元后的上下限与最终结果。</p></div>}</div><div className="shrink-0 border-t border-[#ECEEF3] p-5"><button onClick={()=>setGraded(true)} className="w-full rounded-full bg-[#EEF0FF] py-3 text-[10px] font-semibold text-[#5967D9]">AI 批改</button></div></section>
    </main>
  </div>;

  const isPreview = kind === "physics-lab" || kind === "physics-preview";
  return <div className="fixed inset-0 z-[625] flex h-[100dvh] flex-col bg-[#F3F4F8]">
    <header className="flex h-[68px] shrink-0 items-center border-b border-[#E7E9EF] bg-white px-5"><button onClick={onClose} aria-label="关闭任务" className="grid h-9 w-9 place-items-center rounded-xl hover:bg-[#F4F5F8]"><X size={17}/></button><div className="ml-4"><b className="block text-[14px]">{task.title}</b><small className="text-[10px] text-[#8A909C]">{task.source} · {task.meta}</small></div></header>
    <main className="grid min-h-0 flex-1 grid-cols-[62%_38%]"><section className="flex min-h-0 flex-col border-r border-[#E1E4EB] bg-[#ECEEF4] p-5"><div className="mb-3 flex items-center"><b className="text-[12px]">{isPreview?"预习内容":"复习内容"} · {task.source}</b></div><div className="flex min-h-0 flex-1 items-center justify-center"><div className="relative aspect-[4/3] w-full max-w-[920px] overflow-hidden rounded-2xl shadow-[0_5px_24px_rgba(27,31,48,.10)]"><SourceContent kind={kind} playing={playing} onPlay={()=>setPlaying(!playing)}/></div></div></section>
      <section className="flex min-h-0 flex-col bg-white p-6"><p className="text-[9px] font-bold tracking-[.12em] text-[#9CA3AF]">{isPreview?"本次预习":"本次复习"}</p><h2 className="mt-2 text-[20px] font-bold leading-8">{task.action}</h2><div className="mt-6 rounded-2xl bg-[#F3F5FF] p-5"><div className="flex items-center gap-3"><button onClick={()=>setPlaying(!playing)} className="grid h-12 w-12 place-items-center rounded-full bg-[#4D5CFF] text-white"><CirclePlay size={22}/></button><div><b className="text-[12px]">{isPreview?"预习播客":"复习讲解"}</b><p className="mt-1 text-[9px] text-[#858C9A]">{playing?"正在播放":"点击播放"} · 08:04</p></div></div><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-white"><div className={`h-full rounded-full bg-[#6573FF] transition-all ${playing?"w-[42%]":"w-0"}`}/></div></div><div className="mt-5 rounded-2xl border border-[#E7E9EF] p-5"><b className="text-[12px]">{isPreview?"看完需要知道":"直接复习这里"}</b><div className="mt-4 space-y-3">{task.checks.map((item,index)=><div key={item} className="flex items-start gap-3 text-[10px] leading-5 text-[#596170]"><span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-[#EEF0FF] text-[8px] font-bold text-[#4D5CFF]">{index+1}</span><span>{item}</span></div>)}</div></div><AiConversationModule contextKey={kind} contextLabel={task.source} initialAssistant={isPreview?"我已经带上本次预习内容。你可以边看边问，也可以让我用语音再讲一遍。":"我已定位到这次要复习的内容，会围绕论证转折和高频表达帮助你回忆。"} onAsk={ask} placeholder={task.placeholder} suggestions={isPreview?["讲一下重点","哪些最容易错？"]:["开始复习","再举一个例子"]} className="mt-5 min-h-0 flex-1 rounded-2xl" compact/></section>
    </main>
  </div>;
}
